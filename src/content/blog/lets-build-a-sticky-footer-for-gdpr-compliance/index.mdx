...
_Disclaimer: I'm not a legal expert, so do your own due digilance for what is required for proper GDPR compliance._
...
Before doing anything complex in terms of cookie logic, we first need to know if the component is mounted - only then should we fire the fade animation - otherwise, we may get ugly animation flashing. For this, I use a nice utiliy hook I call `useDidMount`:
