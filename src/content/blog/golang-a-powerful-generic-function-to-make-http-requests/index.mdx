---
title: "Golang: A Powerful Generic Function to Make HTTP Requests"
...
-   POSTing to [DeepL's translate text API](https://www.deepl.com/docs-api/translate-text/)
-   GET / POST to [Instagram's Graph API](https://developers.facebook.com/docs/instagram-api/)
-   GET / POST to [Sendbird's Chat Platform API](https://sendbird.com/docs/chat/v3/platform-api/overview).

I figured I'd share this function with the world, as it's a great real-world example of how to use Go's generic capabilities. I' going to do a full walk-through of how the function is built, but if you just want the function, hop down to the [full code snippet](#full-code-snippet).

For the remainder of this post, I'll assume that we write the generic function in a package called `http_helper`. This is what is used in [the example repository](https://github.com/princefishthrower/golang-generic-http-helper-function.git).
...
- I claim that we can handle GETs, POSTs, and any other HTTP method, we need also accept an HTTP method as a parameter - we called it `httpMethod`, type `string` (see further down why we chose to go with `string`)
...
Now we can handle the GET case, using this url object to add the query parameters:
...
We can create the request, passing the `body` parameter in:
...
Finally getting to the powerful generic part of this function, we used the `encoding/json` package to unmarshal the response data into the type we expect. We throw an error if the `encoding/json` package is unable to unmarshal to the type specified:
...
As we saw in our original implementation, even in the official go typings of `http.NewRequest`, the HTTP method is simply type `string`. The reason we have decided not to implement any check is because the there are only five methods - `GET`, `POST`, `PUT`, `PATCH`, and `DELETE`, and they aren't likely to change at all any time soon! However, it _is_ of course still possible to make typos on the method field when calling this function (i.e. `GET`), so you and your team may decide to make the type a bit more strict. You have two options:

**Option 1. The simple one - a regex**
...
**Option 2. The more involved one - enum option**
...
And we'll have change the if statement to compare against the new `GET` type instead of the string `"GET"`:
...
Option #2 may be a bit of over-engineering, but it's also the most type-safe. It's up to you to decide which one you want to use. As mentioned, our team has decided to forgo any checks on the `httpMethod` parameter, as we do extensive [unit and integration testing](http://localhost:8000/blog/one-year-as-chief-technology-officer-at-inclub-insights-from-startup-land/) on our backend that would catch, by way of request errors, any typos in the http method passed to `MakeHTTPRequest`.

## Thanks!

I hope you enjoyed this post! I'm really loving how amazingly easy it is to build backend infrastructure with golang. I'm looking forward to posting more golang posts in the future.
