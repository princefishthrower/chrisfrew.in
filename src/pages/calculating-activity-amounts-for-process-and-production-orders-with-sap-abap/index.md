...
## You've found the only post on the entire internet that describes the full process of calculating activity amounts for production order confirmations in SAP ABAP! :scream::scream::scream:
## I hope you use this post to maximum effectiveness, because I wasted four days looking for an answer. :smile:

## The Problem
Let's jump write into ABAP programming: the go-to function module to calculate time ticket proposals for production orders is with function module `BAPI_PRODORDCONF_GET_TT_PROP`:
...
Pretty similar. However, as part of my indentured servitude as a full time employee and doing four days of searching, I learned that many developers had issues with these function modules, namely that, while things like date, yield/scrap amounts, and the goods movements would be proposed just fine by these BAPIs, the activity confirmation amount were not always calculated, which are a very important part of the calculation of the confirmation!

So, while the _reason_ the two functions wouldn't _always_ produce activity confirmation amounts automatically, I realized I would have to calculate them in a more manual manner and then add them to those BAPI structures `lt_timetickets` manually.

Through SAP GUI in transaction CO11N, determining system-proposed confirmation amounts is easy enough by telling the system to calculate the formula over the work area<sup><a href="#footnote-3">3</a></sup>. My struggle was finding the programmatic way to do this in ABAP.

## The Calculation

The problem with production planning is that even with very strong ABAP programming experience, the levels and steps with the production planning module can quickly elevate to high complexity program procedures. I have to give credit where credit is due, and received a lot of assistance from our production planning module supervisor - he showed me the transactions that finally got me to a working result.

Anyone with significant production planning experience (i.e., not me :joy: ) knows that the actual formula behind the operation/activity calculation is not with the operation itself, but with the work center _behind_ the operation. And of course, in transaction CR03 (display work center), in the 'Costing' Tab, there is a 'Formula' button with the check symbol on it:

![GUI testing calculation](test-formula.jpeg)

This was my starting point to find any sort of function that could do this programmatically - with more time, I learned from SCN a the function that does exactly that GUI calculation! It's called `CR_WORKCENTER_FORMULA_CALC`!
...
Fine. We'll do it ourselves. We set a breakpoint in the first line of `CR_WORKCENTER_FORMULA_CALC`:
...
:pray: Violá! :pray: We've found how `CR_WORKCENTER_FORMULA_CALC` is called out in the wild!
...
So what the heck does SAP do with this function? Well, first, they are doing a do loop six times. Aha, we know why - because there can be a total of six confirmation values<sup><a href="#footnote-1">1</a></sup>  - SAP is by default looping over all of them.

So, they pass the work center code (`arbid`), the operation number (`rcrop`), the index of the confirmation (a value of 1-6), a list of our 4 default values (`vgw`), and the date (`date`).

In turn, the function spits out the fixed value amount of the activity calculation (`value_fix`), the variable part (`value_var`), both in whatever unit the function gives out in `unit`, and `co_a` is for the cost center accounting<sup><a href="#footnote-4">4</a></sup>. There are actually four exceptions to this function, so I'm not sure why SAP internal is taking only two of them. In any case, there are many other IMPORTING and EXPORTING parameters in the function signature, but this looks like a good starting point as a minimum. In fact, as you'll see below, the final working version of the code I created is almost exactly the same except where I found out with a bit of playing around.
...
" Create goodsmovement structure for all retrograd stuck
...
*       PROD_REL_TOOL =
    ...
    LOOP AT lt_operation INTO ls_operation.
      ...
      DO 6 TIMES. " for each activity (usually 4 activities here at ospelt, but there can be up to 6 activities total)
        ...
        ENDIF.
        CALL FUNCTION 'CR_WORKCENTER_FORMULA_CALC' " CHFE 20.12.207 - you can see a SAP standard example with the function in the source code for transaction CR03 for costing formula: Program SAPLCRA0, Include LCRA0F71, Form FORMEL_RESULT_CALC, line 55
          EXPORTING
            arbid             = ls_afvc-arbid
            op                = ls_op
            vgw               = ls_vgw
        ...
        " CHFE 21.12.2017 - convert any 'H' values - then it is EXACTLY like the SAP standard
        IF lv_unit NE 'MIN'.
          CALL FUNCTION 'UNIT_CONVERSION_SIMPLE'
            ...
            EXCEPTIONS
              ...
              OTHERS               = 10.
          IF sy-subrc <> 0.
            lo_logger->create_message( EXPORTING iv_log_handle = lv_log_handle iv_message = '' ). " CHFE 18.07.2017 - add custom message to message log
            CONCATENATE sy-msgno sy-msgid sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4 INTO es_entityset-error_message.
            APPEND es_entityset TO et_entityset.
          ENDIF.
        ELSE.
      ...
      " CHFE 05.10.2017 - now build the table to confirm the operations (Vorgänge)
      ...
      ls_timetickets-work_cntr   = ls_operation-work_center. " CHFE 18.12.2017 - resource for processauftrage), logdate, logtime, and posting date added to the table
      ...
      ls_timetickets-conf_quan_unit = ls_conf_quan_unit_so-low. " CHFE 06.10.2017 - also the unit
...
Because SAP is ever-so-special, process orders have a slightly different program process due a few name changes. Here is a summary of those name changes:

| Production Order Variable Name | Process Order Variable Name | meaning |
| ph

Likewise, the function module has a slightly different name of: `BAPI`

Here's the code for calculating activities for process orders - _note the slight change in those variable names!_ - and be careful with your types (carefully look over the `DATA:` statement), otherwise you'll get :boom:**exploding runtime errors**:boom: when you try to confirm something with one of our two main functions and you pass, for example, the  `lt_timetickets` with the production type `BAPI_PRODUCTION_BLAAA`, and you pass it into the of the _process_ order function module.
...
Whew. Did it feel like a marathon? :running: Because it felt to _me_ like a marathon all that time I was scratching my head. I hope you all enjoyed and found this post useful for a not-so-often programmed function of this production planning section of SAP.

Another note and quick plug :smile: I hope everyone can appreciate the time I put in to creating these posts (_hint:_ posts this long take a **LONG** time to write!). It would make me happy and give me motivation to keep writing these detailed ABAP posts if you'd consider donating (in the footer of any page on this blog) I have a [paypal.me link](https://www.paypal.me/chrisfrewin), and I'm also an Ethereum developer if you'd like to donate Ether or if you would like to donate some RaiBlocks, as I am a RaiBlocks enthusiest if you would prefer to donate in that way.
...
<div id="footnote-1">
1. Note: At my company, we usually use only up to four confirmation activities. Note that there can be up to six, but for this example, we'll say that the confirmation process has only a maximum of four.
</div>

<div id="footnote-2">
2. Activities are for production orders; Operations are for process orders. From this point onward, I will refer just to activities, since this post focuses on production orders and then I finally post the adjacent code for process orders.
</div>
